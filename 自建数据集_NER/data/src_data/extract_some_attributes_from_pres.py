...
def process_herb(prescriptions_file_path):
    ...
    for prescription in data:
        ...
        if '主治' in prescription:
            herb_set['symptoms_input'] = prescription['主治']
        if '方名' in prescription:
            herb_set['fang'] = prescription['方名']
...
if __name__ == '__main__':
    ...
    for sample in herb_dataset:
        ...
        if 'fang' in sample:
            question_answer_pair['fang'] = sample['fang']
        if 'symptoms_input' in sample:
            question_answer_pair['symptoms_input'] = sample['symptoms_input']
            question_answer_pair['symptoms_output'] = ""
        else:
            continue
        result.append(question_answer_pair)
    
    with open(herb_infer_file_path + '/symptoms_infer.json', 'w', encoding='utf-8') as file:
